...
class GameView {
  ...
  hasConflicts(edge) {
    const edge_vertex1 = edge.vertex1;
    const edge_vertex2 = edge.vertex2;

    for (let i = 0; i < this.FullVertex.length; i++) {
      if (i === this.FullVertex.length - 1) continue;
      const board_vertex1 = this.FullVertex[i];
      const board_vertex2 = this.FullVertex[i + 1];
      ...
      return (
        (convolution.every(value => value >= 0) &&
          convolution.some(value => value > 0)) ||
        (convolution.every(value => value <= 0) &&
          convolution.some(value => value < 0))
      );
    }

...
class GameView {
  constructor(ctx, options) {
    ...
    // this.startingVertex = [];
    this.allVertex = this.initializeStartingVertex();
  ...
  isStartingVertexPos(x, y) {
    for (let i = 0; i < this.allVertex.length; i++) {
      let currentVertex = this.allVertex[i];
  ...
  handleMouseUp(e) {
    ...
    for (let i = 0; i < this.allVertexPos.length; i++) {
      ...
      if (
        ...
        this.allVertex = this.allVertex
          .slice(0, this.index - 1)
          .concat([newVertex])
          .concat(this.allVertex.slice(this.index - 1));

        break;
      }
    }

    console.log(this.allVertex);
  ...
  drawStartingLines() {
    for (let i = 0; i < this.allVertex.length; i++) {
      console.log(this.allVertex.length);
      if (i === this.allVertex.length - 1) continue;

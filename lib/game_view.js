...
class GameView {
  ...
  selfConflicting(prevVertex, newVertex, nextVertex) {
    // return false;
    // const result = [];
    // const vertex_array = [prevVertex, nextVertex, newVertex, newVertex];
    // const convolution = [];
    ...
    // ) {
    //   result.push(true);
    // } else if (convolution.every(value => value < 1200 && value > -1200)) {
    //   for (let i = 0; i < 4; i++) {
    //     const vertex1 = vertex_array[i];
    //     const vertex2 = vertex_array[(i + 1) % 4];
    //     const vertex3 = vertex_array[(i + 2) % 4];

    //     let maxX = prevVertex.x > nextVertex.x ? prevVertex.x : nextVertex.x;
    //     let minX = prevVertex.x < nextVertex.x ? prevVertex.x : nextVertex.x;
    //     let maxY = prevVertex.y > nextVertex.y ? prevVertex.y : nextVertex.y;
    //     let minY = prevVertex.y < nextVertex.y ? prevVertex.y : nextVertex.y;

    //     console.log(vertex3.y, minY, maxY);
    //     console.log('check Y', vertex3.y > maxY || vertex3.y < minY);
    //     if (
    //       (vertex3.x - vertex1.x) * (vertex3.y - vertex2.y) -
    //         (vertex3.x - vertex2.x) * (vertex3.y - vertex1.y) >=
    //         -250 &&
    ...
    const bool1 = this.vertexOnEdge(prevVertex, newVertex, nextVertex);
    const bool2 = this.vertexOnEdge(nextVertex, newVertex, prevVertex);

    return bool1 || bool2;
  }

  vertexOnEdge(edgeVertex1, edgeVertex2, vertex) {
    const x = vertex.x;
    const y = vertex.y;
